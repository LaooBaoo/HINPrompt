...
class PromptEmbedding(torch.nn.Module):
    def __init__(self, config):
        ...
        self.num_prompt = config['num_meta_paths']
        self.embedding_dim = config['prompt_dim']
        self.embedding_prompt = torch.nn.Embedding(
            num_embeddings=self.num_prompt,
            embedding_dim=self.embedding_dim
        )
        self.reset_parameters()
    
    def reset_parameters(self):
        nn.init.xavier_normal_(self.embedding_prompt.weight)
    
    def forward(self, prompt_fea):
        """

        :param prompt_fea: tensor, shape = [#sample, #prompt_fea]
        :return:
        """
        # prompt_idx = Variable(prompt_fea[:, 0], requires_grad=False)

        prompt_emb = self.embedding_prompt(prompt_fea)
        return prompt_emb
...
class UserEmbeddingML(torch.nn.Module):
    ...
    def reset_parameters(self):
        nn.init.xavier_normal_(self.embedding_gender.weight)
        nn.init.xavier_normal_(self.embedding_age.weight)
        nn.init.xavier_normal_(self.embedding_occupation.weight)
        nn.init.xavier_normal_(self.embedding_area.weight)
...
class ItemEmbeddingML(torch.nn.Module):
    ...
    def reset_parameters(self):
        nn.init.xavier_normal_(self.embedding_rate.weight)
        nn.init.xavier_normal_(self.embedding_genre.weight)
...
class UserEmbeddingYelp(torch.nn.Module):
    ...
    def reset_parameters(self):
        nn.init.xavier_normal_(self.embedding_fans.weight)
        nn.init.xavier_normal_(self.embedding_avgrating.weight)    
        nn.init.xavier_normal_(self.embedding_friends.weight)


    def forward(self, user_fea):
        fans_idx = Variable(user_fea[0][:, 0], requires_grad=False)  # [#sample]
...
class ItemEmbeddingYelp(torch.nn.Module):
    ...
    def reset_parameters(self):
        nn.init.xavier_normal_(self.embedding_stars.weight)
        nn.init.xavier_normal_(self.embedding_postalcode.weight)
...
class UserEmbeddingDB(torch.nn.Module):
    ...
    def reset_parameters(self):
        nn.init.xavier_normal_(self.embedding_location.weight)
...
class ItemEmbeddingDB(torch.nn.Module):
    ...
    def reset_parameters(self):
        nn.init.xavier_normal_(self.embedding_publisher.weight)
