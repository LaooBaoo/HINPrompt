...
def random_sampling_normal(df, k,train_df):
    train_data = []
    supp_data = []
    query_data = []
    grouped = df.groupby('user')
    ...
    train_data_df = train_data_df.append(supp_data)
...
def load_movielens():
    metapath = ["new_u_b_df.csv","new_u_b_t_b_df.csv","new_u_b_u_b_df.csv","new_u_b_c_b_df.csv"]
    train_list= []

    with open("./data/yelp/user_feature.pkl", "rb") as f:
        user_feature = pickle.load(f)

    with open("./data/yelp/item_feature.pkl", "rb") as f:
        item_feature = pickle.load(f)

    for path in metapath:
        df = pd.read_csv("./data/yelp/"+path, delimiter=',')
        df = df.rename(columns={'user1': 'user'})
        df = df.rename(columns={'user2': 'user'})
        df = df.rename(columns={'business': 'item'})
    ...
    cold_data = pd.read_csv("./data/yelp/new_"+state+"_df.csv", delimiter=',')
    cold_data = cold_data.rename(columns={'business': 'item'})
    ...
    cold_supp = cold_supp.rename(columns={'business': 'item'})
    cold_query = cold_query.rename(columns={'user1': 'user'})
    cold_query = cold_query.rename(columns={'user2': 'user'})
    cold_query = cold_query.rename(columns={'business': 'item'})
