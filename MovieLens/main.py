...
def prompt(pretrain_model):
    ...
    prompt_model = ColdPrompt(config['embedding_dim'],config['num_meta_paths'],len(user_list),config).to(config["device"])
...
if __name__ == "__main__":
    ...
    #   0.005,0.0002,1000,32,256,4

    config['lr'], config['wd'], config['epochs'], config['embedding_dim'], config['hid_dim'], config['num_meta_paths']\
        = 0.005,0.0002,1000,32,256,4
    ...
    end_time = time.time()
    run_time = end_time - start_time
    # print("train+testing 程序运行时间为:", run_time, "秒")
    # attentionPrompt(model)
